...
// when the package is built with `ADSQL_SIGNPOSTS=1` (a dev/bench profiling build);
...
#if ADSQL_SIGNPOSTS && canImport(os)
    import os

    enum Signposts {
        static let writer = OSSignposter(subsystem: "fr.gcqd.adsql", category: "writer")
        static let reader = OSSignposter(subsystem: "fr.gcqd.adsql", category: "reader")
    }
