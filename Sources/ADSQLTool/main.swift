// The `scan` command walks the raw key/value store through the engine cursor, which
        ...
        case "schema":
            ...
            for name in tableNames {
                ...
                for fk in table.foreignKeys {
                    print(
                        "  FK (\(fk.childColumns.joined(separator: ", "))) → \(fk.parentTable) ON DELETE \(fk.onDelete)"
                    )
                }
