...
extension SelectExecutor {
    ...
    static func finishCompound(
        ...
        if compound.limit != nil || compound.offset != nil {
            let env = SQLEvalEnv.parametersOnly(now: params.now) { p throws(DBError) in try params.lookup(p) }
    ...
    static func sliceBounds(
        ...
        let env = SQLEvalEnv.parametersOnly(now: params.now) { parameter throws(DBError) in try params.lookup(parameter)
        }
