...
///
/// Unlike ``Where`` and ``Having`` (which `AND`-combine when repeated), a
/// repeated `Select` is *last-wins*: the final `Select` replaces the projection.
/// A query has a single projection list, so the last clause is treated as the
/// definitive one rather than silently concatenating columns.
...
/// `HAVING <predicate>`. Repeated `Having` clauses are `AND`-combined, mirroring
/// ``Where``.
public struct Having: QueryComponent {
    ...
    public init(_ build: (ColumnProxy) -> Predicate) { self.predicate = build(ColumnProxy()) }
    public func apply(to select: inout SQLSelect) {
        if let existing = select.having {
            select.having = .binary(.and, existing, predicate.expression)
        } else {
            select.having = predicate.expression
        }
    }
}
