...
struct RelationalModelStore: Sendable {
    struct TableModel: Sendable {
        var definition: TableDefinition
        var rows: [Int64: [Value]] = [:]
    ...
    mutating func createTable(_ definition: TableDefinition) {
        tables[definition.name] = TableModel(definition: definition)
    }

    mutating func dropTable(_ name: String) {
        tables.removeValue(forKey: name)
        indexes = indexes.filter { $0.value.table != name }
    }

    mutating func createIndex(_ definition: IndexDefinition) {
        indexes[definition.name] = definition
    }

    mutating func dropIndex(_ name: String) {
    ...
    func uniqueConflicts(
        ...
        for indexName in indexes.keys.sorted() {
            ...
            for (rowid, existing) in table.rows where rowid != excluding {
                ...
                if zip(zip(candidate, other), collations)
                    .allSatisfy({
    ...
    @discardableResult
    mutating func delete(from name: String, rowid: Int64) -> Bool {
        tables[name]?.rows.removeValue(forKey: rowid) != nil
    }

    // MARK: - Expected orderings

    func sortedRows(_ name: String) -> [(rowid: Int64, values: [Value])] {
        guard let table = tables[name] else { return [] }
    ...
    func indexOrder(_ indexName: String) -> [Int64] {
