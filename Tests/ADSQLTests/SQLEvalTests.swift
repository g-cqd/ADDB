    ...
    func randomExpressions(seed: UInt64) throws {
        ...
        func expression(_ depth: Int) -> String {
            ...
            switch rng.next() % 12 {
                ...
                case 5:
                    return "(\(expression(depth - 1)) \(["AND", "OR"][Int(rng.next() % 2)]) \(expression(depth - 1)))"
...
struct SQLEvalSemanticsTests {
    ...
    @Test func intFloatCompareEdges() {
        ...
        let cases: [(Int64, Double, Int)] = [
            ...
            (9_007_199_254_740_993, 9007199254740992.0, 1),  // 2^53 + 1 vs 2^53
            (9_007_199_254_740_992, 9007199254740993.0, 0),  // RHS rounds to 2^53
