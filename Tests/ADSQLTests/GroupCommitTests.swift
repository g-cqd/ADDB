...
struct GroupCommitTests {
    ...
    @Test func concurrentWritesBatchIntoFewCommits() async throws {
        ...
        // Occupy the writer queue with a gated holder so queued writes pile into a batch behind it; the
        // writer queue is serial, so the batch drains strictly after the holder returns. Opening the
        // gate releases the holder.
        let writes = 100
        var results: Set<Int> = []
        try await withThreadGatedBatch { gate in
            try? db.writeSync { (txn) throws(DBError) in
                try txn.put(Array("blocker".utf8), [1])
                gate.waitUntilOpen()
            }
        } _: { openGate in
            results = try await withThrowingTaskGroup(of: Int.self) { tasks in
                for i in 0 ..< writes {
                    tasks.addTask {
                ...
                // Everything is enqueued behind the holder; open the gate to release the batch.
                openGate()
    ...
    @Test func failedRequestRollsBackOnlyItsDelta() async throws {
        ...
        // Stall the queue with a gated holder so all three requests land in one batch.
        try await withThreadGatedBatch { gate in
            try? db.writeSync { (txn) throws(DBError) in
    ...
    @Test func ftsBufferSurvivesAnotherRequestsRollback() async throws {
        ...
        // Stall the queue with a gated holder so both async writes land in one batch.
        try await withThreadGatedBatch { gate in
            try? db.writeSync { (txn) throws(DBError) in
                try txn.put(Array("warm".utf8), [0])
                gate.waitUntilOpen()
