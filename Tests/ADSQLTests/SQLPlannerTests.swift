...
struct SQLExplainAndSlowQueryTests {
    ...
    func explainSteps() throws {
        ...
        #expect(try db.prepare("SELECT * FROM docs").explainQueryPlan() == ["SCAN docs"])

        let seek = try db.prepare("SELECT id FROM docs WHERE key = 'doc7'").explainQueryPlan()
        #expect(seek.count == 1)
        #expect(seek[0].contains("SEARCH docs USING INDEX u_key"))

        // A join contributes one step per table: the leading scan + the inner's nested-loop access,
        // each named by resolving the join's table index back through the query binding.
        let join =
            try db.prepare(
                "SELECT a.id FROM docs a JOIN docs b ON b.key = a.key"
            )
            .explainQueryPlan()
        #expect(join.count == 2)
    ...
    func slowQueryFires() throws {
        ...
        let stmt = try db.prepare("SELECT id FROM docs WHERE key = 'doc7'")
        let captured = Mutex<[Statement.SlowQuery]>([])
        stmt.setSlowQueryHook(threshold: .zero) { query in captured.withLock { $0.append(query) } }
        _ = try stmt.all()

        let fired = captured.withLock { $0 }
        #expect(fired.count == 1)
        #expect(fired.first?.sql.contains("SELECT id FROM docs") == true)
    ...
    func slowQueryThresholdAndClear() throws {
        ...
        stmt.setSlowQueryHook(threshold: .seconds(3600)) { _ in count.withLock { $0 += 1 } }
        _ = try stmt.all()
        #expect(count.withLock { $0 } == 0)  // a doc lookup is nowhere near an hour

        stmt.setSlowQueryHook(threshold: .zero) { _ in count.withLock { $0 += 1 } }
        _ = try stmt.all()
        #expect(count.withLock { $0 } == 1)

        stmt.clearSlowQueryHook()
        _ = try stmt.all()
        #expect(count.withLock { $0 } == 1)  // unchanged after clear
    }
}
