...
private func leafInsert(_ page: PageBuf, _ key: [UInt8], _ value: [UInt8]) -> Bool {
    key.withUnsafeBytes { k in
        value.withUnsafeBytes { v in
            ...
            return page.withMutableBytes { Node.leafInsert(&$0, at: index, key: k, value: .inline(v)) }
        }
    }
}
...
struct NodeLeafTests {
    @Test func insertSearchRemoveAgainstModel() {
        let page = PageBuf()
        page.withMutableBytes { PageHeader.initialize(&$0, type: .leaf) }
        ...
        for (i, pair) in pairs.enumerated() where i % 2 == 0 {
            pair.key.withUnsafeBytes { k in
                let (index, exact) = Node.search(page.readOnly, key: k)
                #expect(exact)
                page.withMutableBytes { Node.removeCell(&$0, at: index) }
            }
    ...
    @Test func splitKeepsAllCellsSortedAndSeparatorCorrect() {
        ...
        let separator = pendingKey.withUnsafeBytes { k in
            pendingValue.withUnsafeBytes { v in
                ...
                return left.withMutableBytes { l in
                    right.withMutableBytes { r in
                        Node.splitLeafInserting(
    ...
    @Test func splitLeftMayAliasOriginal() {
        ...
        // `left` aliases the original buffer — exactly the production split contract, where the
        // caller snapshots the pre-split page so the source read can't overlap the `left` span's
        // exclusive borrow. Mirror that here: `original` reads from the snapshot, `left` is `page`.
        let original = PageBuf(copying: page.readOnly)
        let separator = newKey.withUnsafeBytes { k in
            [UInt8]([9, 9])
                .withUnsafeBytes { v in
                    let (index, _) = Node.search(original.readOnly, key: k)
                    return page.withMutableBytes { l in
                        right.withMutableBytes { r in
                            Node.splitLeafInserting(
                                original: original.readOnly, at: index, key: k, value: .inline(v),
                                left: &l, right: &r)
                        }
                    }
                }
...
struct NodeBranchTests {
    func makeBranch(_ separators: [(String, UInt64)], leftmost: UInt64) -> PageBuf {
        let page = PageBuf()
        page.withMutableBytes { PageHeader.initialize(&$0, type: .branch) }
        page.withMutableBytes { PageHeader.setLink(&$0, leftmost) }
        for (k, child) in separators {
            key(k)
                .withUnsafeBytes { kb in
                    let (index, exact) = Node.search(page.readOnly, key: kb)
                    precondition(!exact)
                    #expect(page.withMutableBytes { Node.branchInsert(&$0, at: index, key: kb, child: child) })
                }
        }
        return page
    }
    ...
    @Test func setChildInPlace() {
        let page = makeBranch([("g", 11), ("p", 12)], leftmost: 10)
        page.withMutableBytes { Node.branchSetChild(&$0, at: 1, child: 99) }
    ...
    @Test func branchSplitPromotesMiddleKey() {
        let page = PageBuf()
        page.withMutableBytes { PageHeader.initialize(&$0, type: .branch) }
        page.withMutableBytes { PageHeader.setLink(&$0, 1000) }
        var n: UInt64 = 0
        var lastKey = ""
        while true {
            let k = "sep-" + pad(n * 2, 6)
            let ok = key(k)
                .withUnsafeBytes { kb in
                    let (index, exact) = Node.search(page.readOnly, key: kb)
                    precondition(!exact)
                    return page.withMutableBytes { Node.branchInsert(&$0, at: index, key: kb, child: 2000 + n) }
                }
            if !ok {
                lastKey = k
                break
