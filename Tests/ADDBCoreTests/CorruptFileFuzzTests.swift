...
// This seeded fuzz suite depends on ADTestKit's `SeededRNG` and `withTempPath`, both
// DEV-only (resolved only under `ADDB_DEV=1`). It therefore compiles only when ADTestKit
// is importable; in the lean `ADDB_TESTING=1` build it compiles to nothing rather than
// failing to resolve the module. A deterministic, ADTestKit-free corrupted-page rejection
// check (single hand-crafted corruption, no random corpus) lives in
// `StorageEngineCharacterizationTests`.
